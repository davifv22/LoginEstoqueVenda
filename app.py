...
class controle(UserMixin, db.Model):
    ...
    def __init__(self, debito_atual, data_ref):
...
def get_dashboard():
    ...
    data_ref = db.session.query(controle.data_ref).scalar()
    if data_ref is None:
        data_ref = ''
    ...
    if date_query is None:
        data_fechamento = ''
        situacao = 'ABERTO'
        dados = caixa(date, data_fechamento, data_ref, 0, 0, situacao)
    ...
    vrVendaMensal_query = db.session.query(db.func.sum(
        produtos_vendidos.valor_total)).filter_by(data_ref=data_ref, situacao='BAIXADO').scalar()
    ...
    return render_template("dashboard.html", month=data_ref, date=date, user=user, nome=nome_empresa, produtos=todos_produtos, vrDebito=vrDebito, valorVendaMensal=vrVendaMensal, vrCompraMensal=vrCompraMensal, vrTotal=vrTotal)
...
def get_vendas_caixa(data_caixa):
    ...
    dtCaixa_query = db.session.query(
        caixa.data_caixa).filter_by(data_ref=data_ref).all()
    ...
    if clientes is None and produtos == '':
        pass
    page = request.args.get('page', 1, type=int)
    per_page = 100
    ...
    return render_template("vendas.html", qtde_lancados=qtde_lancados, total_lancados=total_lancados, produtos_lancados=produtos_lancados,
                           caixa_query=caixa_query, clientes=clientes, produtos=produtos, date=date, qtde_vendidos=qtde_vendidos, dtCaixa=dtCaixa,
                           vrCaixa=vrCaixa, user=user, nome=nome_empresa)
...
def post_vendas_lancar(data_caixa):
    ...
    if data_ref is None:
        data_ref = ''
    dados_produto = produtos_vendidos(nome_produto, data_ref, nome_cliente, tipo_pagto,
...
def post_baixar_caixa(data_caixa):
    ...
    for venda in venda_query:
        if id_itens == '':
            ...
            id_itens = f'{id_itens}' + f', {venda.id}'
    ...
    db.session.commit()
    for produto_lanc in venda_query:
        id = produto_lanc.id
        quantidade = produto_lanc.quantidade
        produto_estoque = cadprodutos.query.filter_by(id=id).first()
        produto_estoque = produto_estoque.quantidade        
        quantidade = produto_estoque - quantidade
        cadprodutos.query.filter_by(id=id).update({"quantidade": quantidade})
        db.session.commit()
    situacao = 'BAIXADO'
...
def get_parametrizacao():
    ...
    if data_ref is None:
        data_ref = ''
    return render_template("parametrizacao.html", data_ref=data_ref, date=date, vrDebito=vrDebito, user=user, nome=nome_empresa)
...
def post_parametrizacao():
    if controle.query.filter_by(id=1).first() is None:
        ...
        dados_controle = controle(debito_atual, data_ref)
        db.session.add(dados_controle)
        db.session.commit()
    else: 
        debito_atual = request.form.get('debito_atual')
        data_ref = request.form.get('data_ref')
        controle.query.filter_by(id=1).update({"debito_atual": debito_atual, "data_ref": data_ref})
    db.session.commit()
